...
export class Tester {
    ...
    public initiate(logMode: boolean): number {
        ...
        let check = 0;
        try {
            check = Utils.generateSchemas(
                this.tutTd,
                this.testConfig.SchemaLocation,
                logMode
            );
            if (logMode)
                console.log(
        ...
        try {
            ...
            if (logMode)
                console.log('\x1b[36m%s\x1b[0m', '* Finished code generation');
        } catch (Error) {
            if (logMode)
                console.log('Utils.CodeGenerator Initialization Error' + Error);
        }
    ...
    private tryToInvokeAction(actionName: string, toSend?: JSON): Promise<any> {
        if (toSend != null) {
            return Utils.promiseTimeout(
                this.testConfig.ActionTimeout,
                this.tut.invokeAction(actionName, toSend)
            );
        }
        return Utils.promiseTimeout(
            this.testConfig.ActionTimeout,
            this.tut.invokeAction(actionName)
        );
    }
    ...
    public testAction(
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                toSend = self.codeGen.findRequestValue(
                    self.testConfig.TestDataLocation,
                    testScenario,
                    interactionIndex,
                    actionName
                );
                if (logMode)
                    console.log(
                        '\x1b[36m%s%s\x1b[0m',
                        '* Created value to send :',
                        JSON.stringify(toSend, null, ' ')
                    );
            } catch (Error) {
                if (logMode)
                    console.log(
                        '\x1b[36m%s\x1b[0m',
                        '* Cannot create message for ' +
                            actionName +
                            ', look at the previous message to identify the problem'
                    );
                self.testReport.addMessage(
                    testCycle,
                    testScenario,
                    actionName,
                    false,
            ...
            if (toSend != null) {
                const errors: Array<any> = Utils.validateRequest(
                    actionName,
                    toSend,
                    self.testConfig.SchemaLocation,
                    'Action'
                );
                if (errors) {
                    ...
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Created request is not valid for ' +
                                actionName +
                                '\nMessage is ' +
                                toSend +
                                '\nError is ' +
                    ...
                    self.testReport.addMessage(
                        ...
                        JSON.parse('"nothing"'),
                        13,
                        'Created message has bad format: ' +
                    ...
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Created request is valid for: ' + actionName
                        );
                }
            }
            //invoking the action
            try {
                if (logMode)
                    console.log(
                        '\x1b[36m%s%s\x1b[0m',
                        '* Invoking action ' + actionName + ' with data:',
                        JSON.stringify(toSend, null, ' ')
                    );
                // Try to invoke the action.
                self.tryToInvokeAction(actionName, toSend)
                    .then((res: any) => {
                        if (interaction.hasOwnProperty('output')) {
                            ...
                            try {
                                ...
                                self.testReport.addMessage(
                                    ...
                                    answer,
                                    15,
                                    'Response is not in JSON format: ' + error
                                );
                                resolve(true);
                            }
                            //validating the response against its schema, same as before
                            const errorsRes: Array<any> = Utils.validateResponse(
                                actionName,
                                answer,
                                self.testConfig.SchemaLocation,
                                'Action'
                            ...
                            if (errorsRes) {
                                ...
                                if (logMode)
                                    console.log(
                                        '\x1b[36m%s\x1b[0m',
                                        '* Received response is not valid for: ' +
                                            actionName
                                    );
                                self.testReport.addMessage(
                                    testCycle,
                                ...
                                if (logMode)
                                    console.log(
                                        '\x1b[36m%s\x1b[0m',
                                        '* Received response is valid for: ' +
                                            actionName
                                    );
                            }
                            ...
                            if (logMode)
                                console.log(
                                    ...
                                    '* ',
                                    actionName +
                                        ' is successful without return value'
                                );
                            self.testReport.addMessage(
                                testCycle,
                                testScenario,
                ...
                if (logMode)
                    console.log(
                        '* Response receiving for  ' +
                            actionName +
                            'is unsuccessful, continuing with other scenarios'
                    );
                self.testReport.addMessage(
                    testCycle,
                    testScenario,
                    ...
                    JSON.parse('"nothing"'),
                    10,
                    'Problem invoking the action' + Error
                );
                resolve(true);
            }
        });
    ...
    public testProperty(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (logMode && isReadable)
                console.log('\x1b[36m%s\x1b[0m', '* Property is readable');
            if (logMode && !isReadable)
                console.log('\x1b[36m%s\x1b[0m', '* Property not readable');
            if (logMode && isWritable)
                console.log('\x1b[36m%s\x1b[0m', '* Property is writable');
            if (logMode && !isWritable)
                console.log('\x1b[36m%s\x1b[0m', '* Property not writable');
            if (isReadable) {
                const curPropertyData: any = self.tut
                    ...
                    .then((res: any) => {
                        ...
                        if (logMode)
                            console.log(
                                '\x1b[36m%s%s\x1b[0m',
                                '* DATA AFTER FIRST READ PROPERTY:',
                                JSON.stringify(data, null, ' ')
                            );
                        //validating the property value with its Schemas
                        const errorsProp: Array<any> = Utils.validateResponse(
                            propertyName,
                            data,
                            self.testConfig.SchemaLocation,
                            'Property'
                        ...
                        if (!isWritable) {
                            ...
                            if (logMode)
                                console.log(
                                    ...
                                    '* Property test of ' +
                                        propertyName +
                                        ' is successful: no write, first response is schema valid'
                                );
                            self.testReport.addMessage(
                                testCycle,
                                testScenario,
                    ...
                    .catch((error: any) => {
                        ...
                        if (logMode)
                            console.log(
                                '\x1b[36m%s\x1b[0m',
                                '* Problem fetching first time property: ' +
                                    propertyName
                            );
            ...
            if (isWritable) {
                ...
                if (logMode)
                    console.log(
                        '\x1b[36m%s%s\x1b[0m',
                        '* Testing the write functionality for:',
                        propertyName
                    );
                //generating the message to send
                try {
                ...
                const errors: Array<any> = Utils.validateResponse(
                    propertyName,
                    toSend,
                    self.testConfig.SchemaLocation,
                    'Property'
                ...
                if (errors) {
                    ...
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Created request is valid for: ' + propertyName
                        );
                }
                ...
                if (logMode)
                    console.log(
                        '\x1b[36m%s%s\x1b[0m',
                        '* Writing to property ' + propertyName + ' with data:',
                        JSON.stringify(toSend, null, ' ')
                    );
                self.tut
                    ...
                    .then(() => {
                        ...
                        const curPropertyData2: any = self.tut
                            .readProperty(propertyName)
                            .then((res2: any) => {
                                data2 = res2;
                                if (logMode)
                                    console.log(
                                        '\x1b[36m%s%s\x1b[0m',
                                        '* For the second one, gotten property data is:',
                                ...
                                const errorsProp2: Array<any> = Utils.validateResponse(
                                    propertyName,
                                    data2,
                                    self.testConfig.SchemaLocation,
                                    'Property'
                                ...
                                if (errorsProp2) {
                                    ...
                                    if (logMode)
                                        console.log(
                                            '\x1b[36m%s%s\x1b[0m',
                                            '* Received second response is not valid for: ' +
                                                propertyName,
                                            errorsProp2
                                        );
                                    ...
                                    self.testReport.addMessage(
                                        ...
                                        45,
                                        'Received second response is not valid, ' +
                                            JSON.stringify(errorsProp2)
                                    );
                                    resolve(true);
                                } else {
                                    ...
                                    if (logMode)
                                        console.log(
                                            '\x1b[36m%s\x1b[0m',
                                            '* Received second response is valid for: ' +
                                                propertyName
                                        );
                                    if (
                                        JSON.stringify(data2) ==
                                        ...
                                        if (logMode)
                                            console.log(
                                                ...
                                                '* Property test of ' +
                                                    propertyName +
                                                    ' is successful: write works & second get property successful'
                                            );
                                        if (logMode)
                                            console.log(
                                                '\x1b[36m%s\x1b[0m',
                                                '* The value gotten after writing is the same for the property: ' +
                                                    propertyName
                                            );
                                        self.testReport.addMessage(
                                            testCycle,
                                        ...
                                        if (logMode)
                                            console.log(
                                                ...
                                                '* Property test of ' +
                                                    propertyName +
                                                    ' is successful: write works, fetch not matching'
                                            );
                                        self.testReport.addMessage(
                                            testCycle,
                                            testScenario,
                            ...
                            .catch((error: any) => {
                                ...
                                if (logMode)
                                    console.log(
                                        '\x1b[36m%s\x1b[0m',
                                        '* Problem second time fetching property ' +
                                            propertyName +
                                            'in the second get'
                                    );
                                self.testReport.addMessage(
                                    ...
                                    31,
                                    'Could not fetch property in the second get' +
                                        error
                                );
                                reject(true);
                            });
                    ...
                    .catch((error: any) => {
                        if (logMode)
                            console.log(
                                '\x1b[36m%s\x1b[0m',
                                "* Couldn't set the property: " + propertyName
                            );
                        self.testReport.addMessage(
                            testCycle,
                            testScenario,
    ...
    private testInteraction(
        ...
        const self = this;
        return new Promise((resolve, reject) => {
            const interaction = Utils.getInteractionByName(
                self.tutTd,
                interactionName
            );
            console.log(
            ...
            if (interaction[0] == 'Property') {
                ...
                if (logMode)
                    console.log(
                        '\x1b[36m%s\x1b[0m',
                        '* Asked for something other than Action or Property or Event'
                    );
                reject(false);
            }
        });
    ...
    public testScenario(
        ...
        const self = this;
        const interactionList: Array<string> = [];
            ...
            reps.forEach(repNb => {
                promise = promise.then(() => {
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Cycle ' + repNb + ', testing all scenarios'
                        );
            ...
            promise
                .then(() => {
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Testing the Thing has finished without an error'
                        );
                ...
                .catch(() => {
                    if (logMode)
                        console.log(
                            '\x1b[36m%s\x1b[0m',
                            '* Testing the Thing has finished with an error'
                        );
                    reject();
                });
        });
